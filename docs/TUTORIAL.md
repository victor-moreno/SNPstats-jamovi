...
The **SNPstats** jamovi module provides a complete pipeline for single-SNP and multi-SNP genetic association analyses. 

[jamovi](https://www.jamovi.org) is a free and open statistical spreadsheet, built on R, that runs on Windows, macOS and Linux (and in the browser, as jamovi cloud). 

This module extends jamovi's functionality to bring the analyses of the [SNPstats web tool](https://www.snpstats.net) into jamovi, so the data never leaves your computer, datasets are not limited by the web upload, and results can be combined with the rest of jamovi's analyses and with jamovi's data editing, filtering and recoding.
...
The code for this module can be found in [GitHub](https://github.com/victor-moreno/SNPstats-jamovi). Report there any bug, inconsistency, or improvement suggestions.
...
> **Important — the weights file also orders the alleles.** Beyond supplying effect sizes, the weights file defines each SNP's **effect (risk) allele**, which is the allele the dosage counts. Without a file the module cannot know the true risk allele, so it assigns one from the data using the same rule as the SNP Analysis module: the **minor (less frequent) allele** is taken as the effect allele, unless you have reordered the genotype levels in the Jamovi Data panel, in which case that order is respected (the first genotype level's homozygote allele becomes the reference and the other becomes the effect allele). This makes the two modules consistent, but the direction is only a **convention, not a validated risk orientation** — for any given SNP the published risk allele may be the major allele, so a frequency-assigned effect allele can point the opposite way. The unweighted score then sums risk-increasing and risk-decreasing directions together and any real association tends to wash out, which is why an unweighted result can jump from non-significant to significant once a weights file is loaded — the file re-orients every SNP to its true risk allele. **Always load a weights file** (even one with all effect weights set to 1 if you only want an allele count) so the alleles are oriented to the published risk allele; a no-file score is not a genuine polygenic risk score and the result tables carry a warning to that effect.
