...
snpLDHaploClass <- if (requireNamespace("jmvcore", quietly = TRUE)) R6::R6Class(
  ...
  private = list(
    ...
    .run_haplo = function(geno_list, data, response, response_raw, response_type,
                          cov_df, opts, run_haploFreq, run_haploAssoc,
                          run_haploInteraction, run_subpop = FALSE,
                          complete_mask = NULL) {
      ...
      resp_cov_miss    <- rep(FALSE, n_total_rows)
      if (!is.null(response)) resp_cov_miss <- resp_cov_miss | is.na(response)
      if (!is.null(cov_df) && ncol(cov_df) > 0)
        resp_cov_miss <- resp_cov_miss | !complete.cases(cov_df)
      n_miss <- sum(snp_miss_mask) + sum(resp_cov_miss & !snp_miss_mask)
      ...
      if (run_haploAssoc && !is.null(response)) {
        ...
        m_model  <- data.frame(y=y_sub); m_model$geno <- subset_geno(geno_setup, keep)
      ...
      # ── 3. Haplotype × covariate interaction ─────────────────────
