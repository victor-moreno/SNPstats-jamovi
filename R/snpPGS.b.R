...
#   (a) snpGrid has rows  — UI has already been populated; use the grid.
#       This path honours any per-SNP edits the user has made.
#   (b) snpGrid empty, weightsPath set — parse the catalog file directly,
#       then write the resulting rows back to snpGrid via
#       self$options$.set() so that the UI grid populates and subsequent
#       runs enter branch (a).
#   (c) Neither — unit weights (weight = 1) for all selected SNP columns.
#
# NOTE: file reading was removed from events.js because jamovi's JS sandbox
# does not expose window.jamovi.fs.readFile.  R is the sole file-reader.
# ─────────────────────────────────────────────────────────────────────────────
...
snpPGSClass <- R6::R6Class(
  ...
  private = list(
    ...
    #
    # Three branches:
    #   (a) snpGrid has rows  — use the grid (UI path, honours user edits)
    #   (b) snpGrid empty, weightsPath set — parse file, write back to grid
    ...
    .buildWeightTable = function(snpCols, grid) {
      ...
      if (!is.null(grid) && length(grid) > 0) {
        ...
        rows <- lapply(grid, function(r) {
          ...
          #   NULL      — JS null sentinel (not yet from catalog)
          #   numeric 1 — YAML schema default on an unmatched row
      ...
      # ── (b) No grid — parse file directly, then write back to snpGrid ───
      path   <- self$options$weightsPath
      sepOpt <- self$options$weightsSep
      if (!is.null(path) && nchar(trimws(path)) > 0 && file.exists(path)) {
        result <- private$.parseCatalogFile(path, sepOpt, snpCols)
        # Write parsed rows back to the snpGrid option so the UI populates
        # and subsequent runs take branch (a), preserving user edits.
        private$.writeGridToOption(result)
        return(result)
      }
    ...
    # .writeGridToOption
    #
    # Converts a wtable data.frame back to the list-of-lists structure that
    # jamovi expects for an Array option, then calls self$options$.set() to
    # push it into the snpGrid option.  This causes the UI ListBox to
    # populate with the parsed catalog data.
    #
    # effect_weight: NA_real_ → NULL so the JS sentinel logic is preserved
    #   (blankRow uses null; R branch (a) treats null / (matched=F, w=1) as
    #    unset).
    # ════════════════════════════════════════════════════════════════════════
    .writeGridToOption = function(wtable) {
      grid_list <- lapply(seq_len(nrow(wtable)), function(i) {
        r <- wtable[i, ]
        list(
          rsid          = as.character(r$rsid),
          effect_allele = as.character(r$effect_allele),
          other_allele  = as.character(r$other_allele),
          effect_weight = if (is.na(r$effect_weight)) NULL
                          else as.numeric(r$effect_weight),
          chr           = as.character(r$chr),
          pos           = as.character(r$pos),
          matched       = isTRUE(r$matched)
        )
      ...
      tryCatch(
        self$options$.set(snpGrid = grid_list),
        error = function(e) {
          # .set() is not available in all jmvtools versions; fail silently —
          # the results table still shows correct data via .fillSnpGridTable.
          invisible(NULL)
        }
      )
    },
