...
snpStatsClass <- if (requireNamespace("jmvcore", quietly = TRUE)) R6::R6Class(
  ...
  private = list(
    ...
    .run_descriptive = function(data, snp_vars, response_var, response_raw,
      ...
      # ── Visibility ────────────────────────────────────────────────────
      res$covDescGroup$setVisible(run_covDesc && (!is.null(cov_df) || !is.null(response_raw)))
    ...
    .fill_hwe = function(tbl, geno_obj, snp_nm, response_raw, subpop,
      ...
      add_row <- function(key, label, st, miss, p) {
        tbl$addRow(rowKey=key, values=list(
          group=label, n11=as.integer(st["N11"]), n12=as.integer(st["N12"]),
          n22=as.integer(st["N22"]), missing=miss, pval=p))
      }
      st <- hw$statistic
      add_row("All", "All subjects", st, if (isTRUE(show_missing)) total_missing else '', hw$p.value)
